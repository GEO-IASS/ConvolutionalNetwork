...
template <typename T>
void DeLocalResponNormalize(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[], mxClassID classID)
{
    mwSize DATA_DIMS[4];
    mwSize M_DIMS[4];
        ...
        for(mwSize k = 0;k < DATA_DIMS[1];k ++){
            for(mwSize l = 0;l < DATA_DIMS[0];l ++){
                for(mwSize j = 0;j < DATA_DIMS[2];j ++){
                    ...
                    T single_data = data_point[i * data_sample_size + j * data_channel_size + k * DATA_DIMS[0] + l];
                    Output_point[i * data_sample_size + j * data_channel_size + k * DATA_DIMS[0] + l] = (K[0] + alpha[0] * sum_part - 2 * beta[0] * \
                            alpha[0] * pow(single_data,2)) / (pow(K[0] + alpha[0] * sum_part,beta[0] + 1));
                }
            }
        }
    }
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  /***************************************************************************/
  if(debug)
      mexWarnMsgTxt("DeLocalResponNormalize operation!");
...
//     mexErrMsgTxt("Input data and pooling need to be of the same type");
  ...
  if (classID == mxSINGLE_CLASS) {
      ...
      DeLocalResponNormalize<float>(nlhs, plhs, nrhs, prhs, classID);
  }  else if (classID == mxDOUBLE_CLASS) {
      if (debug)
        mexWarnMsgTxt("Executing the double version\n");

      DeLocalResponNormalize<double>(nlhs, plhs, nrhs, prhs, classID);
  }
}